...
test('setup.html mounts lobby controls', async () => {
  ...
  for (const id of ['worlds', 'begin']) assert.ok(d.getElementById(id), `#${id}`);
  for (const a of ['data-mode', 'data-level']) assert.ok(d.querySelector(`[${a}]`), a);
});
test('index links to lobby and APK', async () => {
  const html = await read('index.html');
  assert.ok(html.includes('setup.html'));
  assert.ok(/AlaguliMane\.apk/.test(html));
});
